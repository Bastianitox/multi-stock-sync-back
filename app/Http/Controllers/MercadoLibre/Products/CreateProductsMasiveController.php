...
class CreateProductsMasiveController extends Controller
{
    ...
    public function downloadTemplate($clientId, $categoryId)
    {
        try {
            ...
            Log::error('Error generando plantilla Excel:', ['error' =>  config('app.debug') ? $e->getMessage() : 'Error interno en el servidor.']);
        ...
        foreach ($attributes as $attribute) {
            if (isset($attribute['tags']['required']) && $attribute['tags']['required'] && !empty($attribute['values'])) {
                ...
                foreach ($attribute['values'] as $value) {
                    ...
                    if(isset($attribute['id']) && $attribute['id']== 'size_grid_id'){
                        $genders=
                        $brand=
                        $sizeGridId = $this->getSizeGuides();
                    }
    ...
    private function getSizeGuides(Request $request, $client_id)
    {
