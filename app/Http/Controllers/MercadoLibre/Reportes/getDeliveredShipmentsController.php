...
class getDeliveredShipmentsController
{
    ...
    public function getDeliveredShipments($companyId)
    {
    ...
    private function processOrdersAsync($orders, $accessToken)
    {
        ...
        $pool = new Pool($this->guzzleClient, $requests, [
            ...
            'rejected' => function (RequestException $reason, $index) use (&$responses) {
                $responses[$index] = [
                    'success' => false,
                    'error' =>  config('app.debug') ? $reason->getMessage() : 'Error interno en el servidor.'
                ];
                Log::warning('Petición fallida', ['index' => $index, 'error' =>  config('app.debug') ? $reason->getMessage() : 'Error interno en el servidor.']);
            },
        ]);
