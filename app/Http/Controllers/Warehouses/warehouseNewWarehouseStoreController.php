...
class warehouseNewWarehouseStoreController{
    ...
    public function warehouse_store(Request $request)
    {
        ...
        try {
            ...
            Log::error('Error inesperado:', ['exception' =>  config('app.debug') ? $e->getMessage() : 'Error interno en el servidor.']);
            return response()->json(['message' => 'Error al crear la bodega.', 'error' =>  config('app.debug') ? $e->getMessage() : 'Error interno en el servidor.'], 500);
        }
    }
}