     ...
     */
     ...
     public function company_update(Request $request, $id)
     {
         ...
         try {
             ...
             return response()->json(['message' => 'Empresa no encontrada.', 'error' =>  config('app.debug') ? $e->getMessage() : 'Error interno en el servidor.'], 404);
         } catch (\Exception $e) {
             return response()->json(['message' => 'Error al actualizar la empresa.', 'error' =>  config('app.debug') ? $e->getMessage() : 'Error interno en el servidor.'], 500);
         }
     }
}